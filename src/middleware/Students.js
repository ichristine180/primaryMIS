import db from '../database/connection/query';
 import {checkStudentExist,checkExistOnUpdate} from '../database/queries/Student';
...
async checkExist(req,res,next){
    let student = await db.query(checkExistOnUpdate,[req.body.studentnames,req.body.parentsphonenumber,req.params.id]);
    if(!student.rowCount){
        next();
    }else{
        res.status(400).send({
